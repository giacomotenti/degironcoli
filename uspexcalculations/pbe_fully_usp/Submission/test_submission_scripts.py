...
def test_submission_scripts(whichCluster : int):
    if whichCluster == 1:
        ...
        jobID = submitJob_local(index=INDEX, commnadExecutable=SUBMISSION_SCRIPT)
        print(str(jobID))
        ...
        jobID = submitJob_remote(workingDir=WORKING_DIR, index=INDEX, commnadExecutable=SUBMISSION_SCRIPT)
